...
**Owner:** Phạm Đan Kha  
**Phiên bản:** v0.4  
**Cập nhật:** Input hiện tại là PDF, chưa thể lấy CSV/JSON từ portal.
...
| Export CSV | Green | Có thể export claim-level | Cần trace về PDF gốc |
...
| DRD-004 | Export vẫn là CSV claim-level | Đúng scope MVP |
...
|---|---|---|---|
| OQ-PDF-001 | Một bundle bắt buộc có bao nhiêu file? Answer + Ref + ít nhất 1 Source Content đúng không? | PO/BA | Upload validation |
| OQ-PDF-002 | Source Content PDF sẽ upload từng file hay gộp chung nhiều nguồn? | Engineering/BA | Storage + mapping |
| OQ-PDF-003 | Nếu source URL không parse được, annotator có cần mở nguồn ngoài không? | PO/QA | Source verification |
| OQ-PDF-004 | PDF scan/image có nằm trong MVP không? | PO/Engineering | OCR scope |
| OQ-PDF-005 | Parser dùng rule-based, LLM extraction, hay hybrid? | Engineering/ML | Timeline tuần 1–2 |
