...
- `IAA_SCORE.scope_type`: Cần giữ scope `pair` (để hiển thị bảng IAA theo cặp annotator Ann A ↔ Ann B như Tuyết spec ở Màn hình 12). Để tránh bùng nổ tổ hợp O(N²), Backend chỉ tính toán IAA cho các cặp annotator thực tế có gán overlap trong bảng `IAA_OVERLAP_ASSIGN`.
