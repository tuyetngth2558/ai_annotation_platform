...
export default function ExportView({
  ...
  // Real CSV trigger function mapping approved data dynamically
  const triggerDownload = (job: ExportJob) => {
    ...
    const rows = eligibleTasks.map((t) => {
      const compVal = (t.ann.SF + t.ann.SC + t.ann.NH + t.ann.SQ + t.ann.REL + t.ann.COMP) / 6;
      return [
        `"${t.id}"`,
        `"${t.answerId}"`,
        `"${t.articleCode}"`,
        `"${t.sectionName}"`,
        `"${t.claimOriginal.replace(/"/g, '""')}"`,
        `"${t.claimFinal.replace(/"/g, '""')}"`,
        t.ann.SF.toFixed(2),
        t.ann.SC.toFixed(2),
        t.ann.NH.toFixed(2),
        t.ann.SQ.toFixed(2),
        t.ann.REL.toFixed(2),
        t.ann.COMP.toFixed(2),
        compVal.toFixed(2),
        `"${t.annotator}"`,
        `"${t.sourceStatus}"`,
        `"${t.reason.replace(/"/g, '""')}"`,
    ...
    link.setAttribute("download", `VSF_Annotation_Export_${job.id}_${project.bundleId}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showToast(`Đã xuất và tải về file CSV kết quả cho ${eligibleTasks.length} task thành công!`);
  };
  ...
  const handleCreateExportJob = (e: React.FormEvent) => {
    ...
    showToast(`Đã tạo thành công Export Job mới: ${nextCode}`);
    triggerDownload(newJob);
  };
